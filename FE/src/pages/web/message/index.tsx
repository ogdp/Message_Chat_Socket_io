...
const index = () => {
  ...
  const onHandleGenarateID = (e: any) => {
    ...
    toast.success("Genarate success ID ROOM! 🚀 ", {
  ...
  const fetchNewRoom = async (data: any) => {
    try {
      let config = {
        method: "post",
      ...
      console.log(error);
      toast.error("Error create new room");
    }
  };
  const onHandleNewRoom = async (e: any) => {
    ...
    if (localStorage.getItem("myInfo")) {
      const user: any = localStorage.getItem("myInfo");
      myInfo.userID = JSON.parse(user).userID;
      myInfo.nameUser = JSON.parse(user).nameUser;
    }
    if (userName && userName !== "") {
      myInfo.nameUser = userName;
    }
